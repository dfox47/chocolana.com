...
$(window).on('load', function () {
	...
	$('.js-calc-box-type').click(function () {
		setTimeout(function() {
			currentBoxType();
	...
	$('.calc__layout').find('input').on('blur keyup paste', function (e) {
		...
		if ( e.keyCode === 8 || e.keyCode === 46) {
			...
			node.val(node.val().replace('0', 'о') );

			// allow only cyrillic letters
			// node.val(node.val().replace(/[^А-ЯЁа-яё0-9]/g, '') );

			// allow cyrillic & latin letters
			node.val(node.val().replace(/[^А-ЯЁа-яёA-Za-z0-9]/g, '') );
			...
			if ( node.val().length > 0 ) {
				...
				node.parent().nextAll().not('.calc__box_full').first().find('input').focus();
			}
		}
	});
});
...
function check__calc_box__val() {
	...
	var $calcBtns = $('.calc__btns');

	if ( boxWithMendal > maxSweets ) {
		$calcBtns.find('span[data-btn="4"]').hide();
		$calcBtns.find('span[data-btn="7"]').hide();

		$('.calc__btns__helper').addClass('active');
	}
	else {
		$calcBtns.find('span[data-btn="4"]').show();
		$calcBtns.find('span[data-btn="7"]').show();
...
function currentBoxType() {
	var $calcBoxType    = parseInt( $('.calc__box_type').find('input:checked').parent().attr('data-box-type') );
	var calcLayout      = $('.js-calc-layout');
	var calcPrice       = $('.js-calc-price');

	calcLayout.find('> div').removeClass('active');

	calcLayout.find('> div[data-box-type="' + $calcBoxType + '"]').addClass('active');
